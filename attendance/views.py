...
def home(request):
    ...
    if request.user.profile.user_type == 1:
        ...
        students = ClassStudent.objects.filter(
            classIns__in=Class.objects.filter(assigned_faculty=request.user.profile).values_list('id')).count()
...
def classPage(request):
    ...
    context['classes'] = classes
    return render(request, 'coursemng.html', context)
...
@login_required
def manage_student(request, pk=None, course=None):
    # course = course.objects.all()
    if pk == None:
        student = {}
